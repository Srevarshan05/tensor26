...
const MagicButton = ({ className, href, children }) => {
  const text = "Register";
  const text2 = "Register Now";
  
  return (
    <StyledWrapper className={className}>
      <a href={href} target="_blank" rel="noopener noreferrer" className="btn-link">
            ...
            <div className="txt-wrapper">
              ...
              <div className="txt-2">
                {text2.split('').map((char, index) => (
                  <span key={index} className="btn-letter">{char === ' ' ? '\u00A0' : char}</span>
                ))}
              </div>
            </div>
          </button>
  ...
  @keyframes focused-letter-anim {
    ...
    50% {
      ...
      filter: blur(5px) brightness(150%)
        drop-shadow(-10px 5px 10px hsl(var(--highlight-color-hue), 100%, 70%));
    }
  }
